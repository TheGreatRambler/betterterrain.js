...
this.options.exponent):Math.pow(f,this.options.exponent);this.dataarray[d.getindex(a,c)].h=f/this.getprobableheight()*(this.invert?-1:1)}};h.prototype.getchunkid=function(a,c){return d.getindex(Math.floor(a/this.options.chunksize),Math.floor(c/this.options.chunksize))};h.prototype.chunkexistshere=function(a,c){return this.chunkexists[this.getchunkid(a,c)]?!0:!1};h.prototype.getchunkcoordinates=function(a){a=d.unpair(a);return[a[0]*this.options.chunksize,a[1]*this.options.chunksize]};h.prototype.getmaxheight=
function(){return Math.pow(1.75*this.options.initialripple+1.75*this.options.continentmult,this.options.exponent)};h.prototype.getprobableheight=function(){return(this.options.exponent+1)/(this.options.exponent+2)*Math.pow(1.75*this.options.initialripple+1.75*this.options.continentmult,this.options.exponent)};h.prototype.getprobablemoisture=function(){return 1.75*this.options.moistureweight};h.prototype.setdata=function(a,c){this._initxy(a,c);this.generateheightmap(a,c);this.options.onlyheight||(this.generatemoisture(a,
c),this.generatebiomes(a,c),this.generateentities(a,c),this.options.structuresgen&&this.generatestructures(a,c))};h.prototype.getdata=function(a,c){this.numofelements>this.options.clearmemoryamount&&this._reset();if(!this.chunkexistshere(a,c)){for(var b=this.getchunkcoordinates(this.getchunkid(a,c)),f=0;f<this.options.chunksize;f++)for(var g=0;g<this.options.chunksize;g++)this.setdata(b[0]+f,b[1]+g),this.numofelements++;this.chunkexists[this.getchunkid(a,c)]=!0}return this.dataarray[d.getindex(a,
c)]};return h});
